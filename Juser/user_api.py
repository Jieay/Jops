...
#from saltweb.settings import BASE_DIR, EMAIL_HOST_USER as MAIL_FROM
...
def user_add_mail(user, kwargs):
    ...
    mail_title = u'恭喜你的saltweb用户 %s 添加成功 saltweb' % user.name
    mail_msg = u"""
    Hi, %s
        您的用户名： %s
        您的权限： %s
        您的web登录密码： %s
        您的ssh密钥文件密码： %s
        密钥下载地址： %s/swuser/key/down/?uuid=%s
        说明： 请登陆saltweb后台下载密钥, 然后使用密钥登陆！
...
def server_del_user(username):
    """
    delete a user from saltweb linux system
    删除系统上的某用户
    """
...
def get_display_msg(user, password='', ssh_key_pwd='', send_mail_need=False):
    if send_mail_need:
        ...
        密码：%s <br />
        密钥密码：%s <br />
        密钥下载url: %s/swuser/key/down/?uuid=%s <br />
        该账号密码可以登陆saltweb。
